...
const [previousData, setPreviousData]=useState([]);
 useEffect(() => {
  ...
  axios.get(`http://localhost:5252/api/Provider/GetProvider?id=${userId}`)
    .then((response) => {
    ...
    const precity = localStorage.getItem("city");
const prepincode = localStorage.getItem("pincode");
const predistrict = localStorage.getItem("district");
const preprofession = localStorage.getItem("profession");
    ...
    //     { url: '/plumber.png', name: 'Security Guard' } ];
        ...
        const professionList = [
          ...
          function nav (){
            ...
            navigate("/providerDashboard/personalInfo");
        ...
        //  ----------------
          ...
          console.log("Before calling response",providerResponse);
          ...
          const handleProviderSearch = async (e) => {
            ...
            localStorage.setItem("city",providerFilter.city);
            localStorage.setItem("pincode",providerFilter.pincode);
            localStorage.setItem("district",providerFilter.district);
            localStorage.setItem("profession",providerFilter.profession);
            try {
...
>
  ...
  <div className="grid1">
    {photos.map((photo, index) => (
      <button
        key={index}
        type="button" // Change to "button" if you don't want form submission
        className="box1 btn button11p"
        id={`prosession${index}`} 
        name="procession"
        value={photo.name}
        onClick={(e) =>
          handleProviderSearch1(e, userDetails.city, userDetails.pinCode, userDetails.district, photo.name)
        }
        style={{ backgroundImage: `url(${photo.url})` }}
      >
        <p id="p1">{photo.name}</p>
